...
class DependenceFitter2_bending:
    """
    A model to fit the dependence of tau on param1 and param2, incorporating the effects of 
    observation baseline and cadence. The fitting is performed using MCMC sampling.
    ...
    redshift : float, optional (default=0.)
        Redshift of the sources.
    """
    def __init__(self, tau, baseline, n_cadence, param1, param2, param1_e=0., param2_e=0., redshift=0., s_k=0.1):
        self.tau = tau
        self.baseline = baseline
        self.n_cadence = n_cadence
        self.param1 = param1
    ...
    def log_likelihood(self, theta):
        ...
        float
            Log-likelihood value.
        """
        slope1, slope2, point, intercept, noise = theta
        # s = sigmoid(self.param2 - point, self.s_k)
        # tau_m = slope1* self.param1 + (self.param2 - point) * (slope2* (1-s)) +intercept + np.log10(1+self.redshift)
        tau_m = slope1* self.param1 + softplus(slope2 * (self.param2 - point), s_k=self.s_k) + intercept + np.log10(1+self.redshift)
        tau1_me, tau1_err = tau0_to_tau1(tau_m, self.baseline, self.n_cadence)
        if np.any(np.isnan(tau1_me)):
            return -np.inf
        sigma2 = tau1_err**2 + (noise**2 + (slope1*self.param1_e)**2)* softplus_derivative(tau_m- np.log10(self.baseline) + 0.511636601229464, 0.4239987587065787) ** 2 
        return -0.5 * np.sum((self.tau - tau1_me) ** 2 / sigma2 + np.log(sigma2))
    def log_prior(self, theta):
        """
        Define the prior probability distribution for the parameters.
        ...
        """
        slope1, slope2, point, intercept, noise = theta
        if -2 < slope1 < 2 and -2 < slope2 < 8 and 3 < point < 4 and 0 < noise:
            return 0.0
        return -np.inf
    ...
    def fit(self):
        ...
        flat_samples : array
            Posterior samples of model parameters.
        """
        # Set an initial guess for parameters with a strong prior

        initial = np.array([0.6, 2., 3.4, -24., 0.1])+ 0.01 * np.random.randn(5)
        pos = initial + 1e-4 * np.random.randn(16, 5)
        nwalkers, ndim = pos.shape
        # Run MCMC sampling using multiprocessing
        with Pool(8) as pool:
            sampler = emcee.EnsembleSampler(
